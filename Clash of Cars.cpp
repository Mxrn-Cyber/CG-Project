...
public:
    Color color, tolor;
    double height = 5.5, width = 3, speed, x, y;
    bool forward;
    Car() { renovate(); }
...
void display()
{
    ...
    {
        glColor3d(.7, .7, .7);
        glRectd(Ortho_left, Ortho_top - 1, Ortho_left + 10, Ortho_top - 3);
...
boolean rectRect(double r1x, double r1y, double r2x, double r2y)
{
    ...
    if (r1x + r1w >= r2x + EPS && r1x + EPS <= r2x + r2w && r1y + r1h >= r2y + EPS && r1y + EPS <= r2y + r2h)
        return true;
    return false;
}
...
{
    for (auto c : Cars)
        if (rectRect(c.x, c.y, PlayerCar.x, PlayerCar.y))
            return true;
    return false;
}
...
void reshape(GLsizei width, GLsizei height)
{
    ...
    if (width >= height)
        glOrtho(Ortho_left * aspect, Ortho_right * aspect, Ortho_bottom, Ortho_top, 0, Ortho_right);
    else
        glOrtho(Ortho_left, Ortho_right, Ortho_bottom / aspect, Ortho_top / aspect, 0, Ortho_right);
}
